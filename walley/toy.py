...
TO_RUN="(stms (= display print) (= begin stms) (= list (lambda (. args) (if (null? args) (quote ()) (cons (car args) (apply list (cdr args)))))) (= + (lambda (. args) (if (null? args) 0 (__ADD__ (car args) (apply + (cdr args)))))) (= - (lambda (. args) (if (null? args) 0 (__MINUS__ (car args) (apply - (cdr args)))))) (= * (lambda (. args) (if (null? args) 1 (__MULT__ (car args) (apply * (cdr args)))))) (= / (lambda (. args) (if (null? args) 1 (__DIV__ (car args) (apply / (cdr args)))))) (= null? (lambda (a) (if (eq a (quote ())) 1 0))) (= empty? null?) (= == (lambda (. args) (cond ((__EQUAL__ 2 (len args)) (__EQUAL__ (car args) (car (cdr args)))) (1 (__AND__ (__EQUAL__ (car args) (car (cdr args))) (apply == (cdr args))))))) (= < (lambda (. args) (if (== 2 (len args)) (__LT__ (car args) (car (cdr args))) (__AND__ (__LT__ (car args) (car (cdr args))) (apply < (cdr args)))))) (= <= (lambda (. args) (if (== 2 (len args)) (__OR__ (apply == args) (apply < args)) (__AND__ (__OR__ (apply == (list (car args) (car (cdr args)))) (apply < (list (car args) (car (cdr args))))) (apply <= (cdr args)))))) (= __GT__ (lambda (a b) (if (apply <= (list a b)) 0 1))) (= > (lambda (. args) (if (== 2 (len args)) (__GT__ (car args) (car (cdr args))) (__AND__ (__GT__ (car args) (car (cdr args))) (apply > (cdr args)))))) (= >= (lambda (. args) (if (== 2 (len args)) (or (apply == args) (apply > args)) (__AND__ (__OR__ (apply == (list (car args) (car (cdr args)))) (apply > (list (car args) (car (cdr args))))) (apply >= (cdr args)))))) (= != (lambda (. args) (if (== 2 (len args)) (not (== (car args) (car (cdr args)))) (__AND__ (not (== (car args) (car (cdr args)))) (apply != (cdr args)))))) (= __AND__ (lambda (a b) (if a (if b 1 0) 0))) (= __OR__ (lambda (a b) (if a 1 (if b 1 0)))) (= and (lambda (. args) (if (null? args) 1 (__AND__ (car args) (apply and (cdr args)))))) (= or (lambda (. args) (if (null? args) 0 (__OR__ (car args) (apply or (cdr args)))))) (= not (lambda (a) (if a 0 1))) (= nil 0) (= while (lambda (&judge &stm) (cond (judge (stms stm (while judge stm))) (1 nil)))) (= if (lambda (&condition &stm1 &stm2) (cond (condition stm1) (1 stm2)))) (= reminder (lambda (a b) (if (< a b) a (reminder (- a b) b)))) (= % reminder) (= list? (lambda (a) (if (atom? a) 0 1))) (= factorial (lambda (a) (cond ((__EQUAL__ 1 a) 1) (1 (__MULT__ a (factorial (__MINUS__ a 1))))))) (= ^ (lambda (a b) (if (== b 1) a (* a (^ a (- b 1)))))) (= ** ^) (= +1 (lambda (x) (+ x 1))) (= last (lambda (__list__) (if (null? __list__) (print \"Error...Cannot get last atom of empty list\") (if (null? (cdr __list__)) (car __list__) (last (cdr __list__)))))) (= list-length (lambda (_list_) (if (list? _list_) (if (null? _list_) 0 (+ 1 (list-length (cdr _list_)))) (print \"Error...Function list-length can not be used to get length of non-list type value\")))) (= len list-length) (= list-get (lambda (_list_ index) (if (>= index (list-length _list_)) (print \"Error...Index out of range\") (if (== index 0) (car _list_) (list-get (cdr _list_) (- index 1)))))) (= list-append (lambda (a b) (stms (local= output (cons b (quote ()))) (local= i (- (list-length a) 1)) (while (>= i 0) (stms (local= output (cons (list-get a i) output)) (local= i (- i 1)))) output))) (= range (lambda (arg0 . args) (stms (cond ((== args (quote ())) (stms (local= begin 0) (local= end arg0) (local= interval 1) (local= output (quote ())))) ((== 1 (list-length args)) (stms (local= begin arg0) (local= end (car args)) (local= interval 1) (local= output (quote ())))) (1 (stms (local= begin arg0) (local= end (car args)) (local= interval (car (cdr args))) (local= output (quote ()))))) (if (> interval 0) (while (< begin end) (stms (local= output (list-append output begin)) (local= begin (+ begin interval)))) (while (> begin end) (stms (local= output (list-append output begin)) (local= begin (+ begin interval))))) output))) (define abs (lambda (a) (cond ((< a 0) (- 0 a)) (1 a)))) (define __SQRT_ACCURATE__ 0.001) (define sqrt-iter (lambda (guess x) (cond ((good-enough? guess x) guess) (1 (sqrt-iter (improve guess x) x))))) (define improve (lambda (guess x) (average guess (/ x guess)))) (define average (lambda (x y) (/ (+ x y) 2))) (define good-enough? (lambda (guess x) (< (abs (- (* guess guess) x)) __SQRT_ACCURATE__))) (define sqrt (lambda (x) (sqrt-iter 1.0 x)))) "
